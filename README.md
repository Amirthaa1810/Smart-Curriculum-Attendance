...
Teacher login → teacher@demo.com / teacher123
Generate QR → pick CSE 2nd Year B → Data Structures → Period 1 → Generate QR
Student login (new browser/incognito) → student@demo.com / student123
Scan Attendance → scan the QR with the camera (or Manual Entry)
✓ Attendance Marked Successfully → student's % updates instantly
Teacher side → live marks appear on the Generate QR page & Dashboard
Timetable → opens weekly grid, today highlighted
Free Periods → system detects the 11:00 free period
Daily Planner → personalized plan: Revise Management notes (high priority — weak subject), Preview Data Structures, Practice problems
Suggestions → 3 activity suggestions with durations
Analytics → donut, bar, line charts + warnings
🏗️ Architecture
┌─────────────────┐   HTTP/JWT    ┌──────────────────────┐
...
Backend → Render
Create a Render Web Service from backend/ (Root Directory: backend).
Build: npm install, Start: npm start.
Set env: MONGO_URI, JWT_SECRET, PORT, FRONTEND_URL.
Database → MongoDB Atlas
Create a free cluster → Database User → get the connection string.
Use it as MONGO_URI (with retryWrites=true&w=majority).
Whitelist IPs (or 0.0.0.0/0 for a demo).
